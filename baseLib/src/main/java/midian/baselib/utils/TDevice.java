...
public class TDevice {
    ...
    public static int getDefaultLoadFactor() {
        if (_loadFactor == null) {
            Integer integer = Integer.valueOf(0xf & AppContext.context().getResources().getConfiguration().screenLayout);
            _loadFactor = integer;
    ...
    public static DisplayMetrics getDisplayMetrics() {
        ...
        ((WindowManager) AppContext.context().getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay().getMetrics(displaymetrics);
    ...
    public static int getStatusBarHeight() {
        ...
        try {
            ...
            return AppContext.context().getResources().getDimensionPixelSize(x);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
    ...
    public static String getUdid() {
        String udid = AppContext.context().getProperty("udid");
        if (udid.length() == 0) {
//            SharedPreferences.Editor editor = AppContext.getPreferences().edit();
            udid = String.format("%s", UUID.randomUUID());
            AppContext.context().setProperty("udid",udid);
...
//            editor.commit();
    ...
    public static final boolean hasCamera() {
        if (_hasCamera == null) {
            PackageManager pckMgr = AppContext.context().getPackageManager();
...
//        boolean flag;
//        if (((ConnectivityManager) AppContext.context().getSystemService("connectivity")).getActiveNetworkInfo() != null)
//            flag = true;
//        else
//            flag = false;
//        return flag;
//    }
    ...
    public static boolean isPackageExist(String pckName) {
        try {
            PackageInfo pckInfo = AppContext.context().getPackageManager().getPackageInfo(pckName, 0);
            if (pckInfo != null)
                return true;
...
//            KLog.e(e.getMessage());
    ...
    public static void hideSoftKeyboard(View view) {
        if (view == null)
            return;
        ((InputMethodManager) AppContext.context().getSystemService(Context.INPUT_METHOD_SERVICE)).hideSoftInputFromWindow(view.getWindowToken(), 0);
    }
    ...
    public static void showSoftKeyboard(View view) {
        ((InputMethodManager) AppContext.context().getSystemService(Context.INPUT_METHOD_SERVICE)).showSoftInput(view, InputMethodManager.SHOW_FORCED);
    }

    public static void toogleSoftKeyboard(View view) {
        ((InputMethodManager) AppContext.context().getSystemService(Context.INPUT_METHOD_SERVICE)).toggleSoftInput(0, InputMethodManager.HIDE_NOT_ALWAYS);
    }
    ...
    public static String getCurCountryLan() {
        return AppContext.context().getResources().getConfiguration().locale.getLanguage() + "-" + AppContext.context().getResources().getConfiguration().locale.getCountry();
    }

    public static boolean isZhCN() {
        String lang = AppContext.context().getResources().getConfiguration().locale.getCountry();
...
//            AppContext.showToast("你手机中没有安装应用市场！");
    ...
    public static PackageInfo getPackageInfo(String pckName) {
        try {
            return AppContext.context().getPackageManager().getPackageInfo(pckName, 0);
        } catch (NameNotFoundException e) {
//            KLog.e(e.getMessage());
        }
        return null;
    ...
    public static int getVersionCode() {
        int versionCode = 0;
        try {
            versionCode = AppContext.context().getPackageManager().getPackageInfo(AppContext.context().getPackageName(), 0).versionCode;
    ...
    public static int getVersionCode(String packageName) {
        int versionCode = 0;
        try {
            versionCode = AppContext.context().getPackageManager().getPackageInfo(packageName, 0).versionCode;
        } catch (PackageManager.NameNotFoundException ex) {
            versionCode = 0;
        }
        return versionCode;
    ...
    public static String getVersionName() {
        String name = "";
        try {
            name = AppContext.context().getPackageManager().getPackageInfo(AppContext.context().getPackageName(), 0).versionName;
        } catch (PackageManager.NameNotFoundException ex) {
            name = "";
        }
        return name;
    ...
    public static boolean isScreenOn() {
        PowerManager pm = (PowerManager) AppContext.context().getSystemService(Context.POWER_SERVICE);
    ...
    public static String getIMEI() {
        TelephonyManager tel = (TelephonyManager) AppContext.context().getSystemService(Context.TELEPHONY_SERVICE);
    ...
    public static void openApp(Context context, String packageName) {
        Intent mainIntent = AppContext.context().getPackageManager().getLaunchIntentForPackage(packageName);
        if (mainIntent == null) {
...
//        AppContext.showToast("复制成功");
    ...
    public static int getStatuBarHeight() {
        ...
        try {
            ...
            field = c.getField("status_bar_height");
            x = Integer.parseInt(field.get(obj).toString());
            sbar = AppContext.context().getResources().getDimensionPixelSize(x);
    ...
    public static int getNetworkType() {
        int netType = 0;
        ConnectivityManager connectivityManager = (ConnectivityManager) AppContext.context().getSystemService(Context.CONNECTIVITY_SERVICE);

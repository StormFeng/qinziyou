package midian.baselib.widget.cardview;
...
public class DiscoverContainerView extends RelativeLayout implements
            ...
            for (int i = 1; i < 5; i++) {
                ...
                if (colorId != null) {
                   /* int margin = i;
                    ...
                    mSlidingCard.initView(colorId);//传入控件需要赋值的参数：colorId为从dataList中获取的颜色》用于渲染viewpager页面
                    View contentView = mSlidingCard.getContentView();
                    LayoutParams params = new LayoutParams(
                            contentView.getLayoutParams());
                    /*params.topMargin = (margin - 1) * getResources().getDimensionPixelSize(R.dimen.card_item_margin);
                    params.leftMargin = margin * getResources().getDimensionPixelSize(R.dimen.card_item_margin_left_right);
                    params.rightMargin = margin * getResources().getDimensionPixelSize(R.dimen.card_item_margin_left_right);*/
    ...
    @Override
    public void onPageScrolled(SlidingCard v, int position,float positionOffset, int positionOffsetPixels) {
        if (positionOffset > 0) {
            operatType = TYPE_LEFT;
            Log.i("页面滑动距离1：positionOffset > 0：：", positionOffset+"");
        } else if (positionOffset < 0) {
            operatType = TYPE_RIGHT;
            Log.i("页面滑动距离2：positionOffset < 0：：", positionOffset+"");
        }
    ...
    @Override
    public void onPageSelectedAfterAnimation(SlidingCard v, int prevPosition,int curPosition) {
        if (activity != null) {
            ...
            if (colorId != null) {
                ...
                View contentView = mSlidingCard.getContentView();
				/*LayoutParams params = new LayoutParams(contentView.getLayoutParams());
				params.topMargin = 2 * getResources().getDimensionPixelSize(R.dimen.card_item_margin);
				params.leftMargin = 3 * getResources().getDimensionPixelSize(R.dimen.card_item_margin_left_right);
				params.rightMargin = 3 * getResources().getDimensionPixelSize(R.dimen.card_item_margin_left_right);
				contentView.setLayoutParams(params);*/
                mSlidingCard.setSlidingMode(SlidingCard.SLIDINGMODE_LEFT_RIGHT);
                mSlidingCard.setCurrentItem(1, false);
                mSlidingCard.setOnPageChangeListener(this);
            ...
            Log.e("test", "onPageSelectedAfterAnimation:" + curPosition + ","+ getChildCount());
        }
    }
